...
//switches
int _filmgrainOn;        //VHS_FILMGRAIN_ON
int _lineNoiseOn;        //VHS_LINENOISE_ON
int _tapeNoiseOn;        //VHS_TAPENOISE_ON
int _signalNoiseOn;      //VHS_YIQNOISE_ON
int _twitchHOn;          //VHS_TWITCH_H_ON
int _twitchVOn;          //VHS_TWITCH_V_ON  
int _jitterHOn;          //VHS_JITTER_H_ON
int _jitterVOn;          //VHS_JITTER_V_ON 
int _linesFloatOn;       //VHS_LINESFLOAT_ON
int _scanLinesOn;        //VHS_SCANLINES_ON
int _stretchOn;          //VHS_STRETCH_ON 
...
// #if VHS_SCANLINES_ON
   ...
   float scanLines(float2 p, float t){
      ...
      if(_linesFloatOn) // #if VHS_LINESFLOAT_ON
...
half3 vhs( Varyings input ){
   ...
   if(_twitchVOn) //#if VHS_TWITCH_V_ON
      p = twitchVertical(.5*twitchVFreq, p, t); 

   if(_twitchHOn) //#if VHS_TWITCH_H_ON
   ...
   if(_stretchOn){ //#if VHS_STRETCH_ON
      p = stretch(p, t, 15., 1., .5, 0.);
      p = stretch(p, t, 8., 1.2, .45, .5);
      p = stretch(p, t, 11., .5, -.35, .25); //up         
   } //#endif
   
   if(_jitterHOn) //#if VHS_JITTER_H_ON
      p = jitterH(p, t);
   
   if(_tapeNoiseOn) //#if VHS_TAPENOISE_ON
      p = tapeNoiseUV(p, pn);
   ...
   if(_jitterVOn){ //#if VHS_JITTER_V_ON
   ...
   if(_tapeNoiseOn || _lineNoiseOn || _filmgrainOn) //#if VHS_TAPENOISE_ON || VHS_LINENOISE_ON || VHS_FILMGRAIN_ON 
      colN = LOAD_TEXTURE2D_X(_TapeTex, pn*_ResN.xy ).xyzw;      
   
   if(_lineNoiseOn || _filmgrainOn) //#if VHS_LINENOISE_ON || VHS_FILMGRAIN_ON
      signal.x += colN.z;
      // signal.x += tex2D(_TapeTex, pn).z;
    
   
   if(_signalNoiseOn) //#if VHS_YIQNOISE_ON
   ...
   if(_signalTweakOn) //#if VHS_SIGNAL_TWEAK_ON
   ...
   if(_ditherOn) // #if VHS_DITHER
      dm = ditherMask(p); 

   //amount of colors per channel
   float3 dv = float3(255,255,255);//.xxx;
     
   if(_colorOn) //#if VHS_COLOR
      dv = colorDec(col); //color decimation      
   


   
   if(_colorOn || _ditherOn){ //#if VHS_COLOR || VHS_DITHER
   ...
   if(_paletteOn) //#if VHS_PALETTE
      col = palette(col);
   

   //TODO put it into 2nd pass?   
   if(_scanLinesOn) //#if VHS_SCANLINES_ON
      col *= scanLines(po, t);             
