Shader "Hidden/Shader/VHSScanlinesEffect_RLPRO"
{
		...
		float4 FragHD(Varyings i) : SV_Target
	{
		...
		float displacement = ((_Time.x / 4 * 1000) * speed) % _ScreenParams.y;
		float ps;
		i.texcoord.y = frac(i.texcoord.y + cos((coord.x + _Time.x / 4) * 100)  * _OffsetDistortion*0.1);
		...
		float4 FragVD(Varyings i) : SV_Target
	{
		...
		float displacement = ((_Time.x / 4 * 1000) * speed) % _ScreenParams.y;
		float ps;
		i.texcoord.x = frac(i.texcoord.x + cos((coord.y + (_Time.x / 4)) * 100)  * _OffsetDistortion*0.1);
