Shader "Hidden/Shader/TV_RLPRO_HDRP"
{
	...
	float3 Fetch(float2 pos, float2 off)
	{
		...
		return SAMPLE_TEXTURE2D_X_LOD(_InputTexture, s_linear_clamp_sampler, ClampAndScaleUVForPoint(pos),0.0).rgb;
	}
	...
	uint2 positionSS = input.texcoord * _ScreenParams.xy ;
		...
		float2 pos1 = Warp(fragCoord.xy  / _ScreenParams.xy );
		fragColor.rgb = Tri(pos1) * Mask(fragCoord);
