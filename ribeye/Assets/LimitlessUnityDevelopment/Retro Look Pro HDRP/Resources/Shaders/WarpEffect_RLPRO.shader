Shader "Hidden/Shader/WarpEffect_RLPRO"
{
		...
		float4 Frag0(Varyings i) : SV_Target
	{
        ...
        float4 col2;
        if(clamp>0)
            col2 = SAMPLE_TEXTURE2D_X_LOD(_InputTexture, s_point_clamp_sampler, ClampAndScaleUVForPoint(pos), 0.0);
        else
        ...
        {
            #if ALPHA_CHANNEL
                        float alpha_Mask = step(0.0001, SAMPLE_TEXTURE2D(_Mask, sampler_Mask, i.texcoord.xy).a);
            #else
                        float alpha_Mask = step(0.0001, SAMPLE_TEXTURE2D(_Mask, sampler_Mask, i.texcoord.xy).r);
            #endif
            fade *= alpha_Mask;
        }
