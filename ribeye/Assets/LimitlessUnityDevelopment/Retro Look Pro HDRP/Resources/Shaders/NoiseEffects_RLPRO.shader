	...
	float4 NoiseFrag(Varyings i) : SV_Target
	{
		...
		signal = rgb2yiq(col);
			...
			for (uint j = 0; j < tailLength % 1023; j++) 
			{
				...
				tn = SAMPLE_TEXTURE2D_X_LOD(_TapeTex, s_linear_clamp_sampler, ClampAndScaleUVForPoint(d), 1).x;
		...
		half4 col1 = LOAD_TEXTURE2D_X(_InputTexture, i.texcoord *_ScreenParams.xy);
